...
Additionally, pre-built binaries for Linux (amd64) are automatically generated by our CI for each release and can be downloaded from the [GitHub Releases page](https://github.com/YOUR_OWNER/YOUR_REPO/releases). Builds from pushes to the `main` branch are also available as development artifacts from the [Actions tab](https://github.com/YOUR_OWNER/YOUR_REPO/actions) (retained for a short period).
