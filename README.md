...
Additionally, pre-built binaries for Linux (amd64) are generated by the "Build and test Jextract" GitHub Actions workflow.
- For official releases, these archives (`jextract-linux-amd64.tar.gz`) can be downloaded from the [GitHub Releases page](https://github.com/YOUR_OWNER/YOUR_REPO/releases).
- Development builds from pushes to the `main` branch can be downloaded as artifacts from the GitHub Actions tab, under the "Build and test Jextract" workflow runs (retained for a short period).
